...
func Open(d string) (*DB, error) {
	...
	db, err := sql.Open("sqlite", filepath.Join(d, "wrangler.db")+"?_journal_mode=WAL&_busy_timeout=5000")
	if err != nil {
		return nil, err
	}
...
func (d *DB) Create(e *Worker) error {
	e.ID = genID()
	e.CreatedAt = now()
	_, err := d.db.Exec(`INSERT INTO workers(id,name,type,command,instances,status,pid,restart_count,last_start_at,created_at)VALUES(?,?,?,?,?,?,?,?,?,?)`, e.ID, e.Name, e.Type, e.Command, e.Instances, e.Status, e.PID, e.RestartCount, e.LastStartAt, e.CreatedAt)
	return err
}
func (d *DB) Get(id string) *Worker {
	var e Worker
	if d.db.QueryRow(`SELECT id,name,type,command,instances,status,pid,restart_count,last_start_at,created_at FROM workers WHERE id=?`, id).Scan(&e.ID, &e.Name, &e.Type, &e.Command, &e.Instances, &e.Status, &e.PID, &e.RestartCount, &e.LastStartAt, &e.CreatedAt) != nil {
		return nil
	}
	return &e
}
func (d *DB) List() []Worker {
	rows, _ := d.db.Query(`SELECT id,name,type,command,instances,status,pid,restart_count,last_start_at,created_at FROM workers ORDER BY created_at DESC`)
	if rows == nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var e Worker
		rows.Scan(&e.ID, &e.Name, &e.Type, &e.Command, &e.Instances, &e.Status, &e.PID, &e.RestartCount, &e.LastStartAt, &e.CreatedAt)
		o = append(o, e)
	}
	return o
}
func (d *DB) Update(e *Worker) error {
	_, err := d.db.Exec(`UPDATE workers SET name=?,type=?,command=?,instances=?,status=?,pid=?,restart_count=?,last_start_at=? WHERE id=?`, e.Name, e.Type, e.Command, e.Instances, e.Status, e.PID, e.RestartCount, e.LastStartAt, e.ID)
	return err
}
