...
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{if s.db.Count()>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/wrangler/");return}};var e store.Worker;json.NewDecoder(r.Body).Decode(&e);if e.Name==""{we(w,400,"name required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
patch.Type=existing.Type};if patch.Command==""{
patch.Command=existing.Command};if patch.Status==""{
patch.Status=existing.Status};if patch.LastStartAt==""{
patch.LastStartAt=existing.LastStartAt}
    if patch.Instances==0{patch.Instances=existing.Instances};if patch.PID==0{patch.PID=existing.PID};if patch.RestartCount==0{patch.RestartCount=existing.RestartCount}
